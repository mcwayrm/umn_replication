...
# Directories 
########################s#
path_data = "E:/umn_replication/original-data/data/main_study_cleaned.dta" # original data 
...
This is strange given the sample. They survey economists and ask them if they would publish a paper. 
This is measured on a sliding scale of 0 to 100. They provide each person with four of five vignettes. 
The authors take the vigenttes from hypothetical studies that are statistically significant. 
...
# Column 1
col1 <- plm(publish ~ low + exlow + exhigh + field + phd + unilow + pval, data = df, index = c("id", "vignette"), model = "within")
col1_se <- sqrt(diag(vcovHC(col1, type = "HC1", cluster = "group")))
...
stargazer(col1,
        ...
        out = paste0(path_out, "table3_col1.tex")) 
```
...
ggplot(df_values, aes(x = zscore)) +
    ...
    theme_minimal() 
# Save the plot
ggsave(filename = paste0(path_out, "z-curve-density.png"), plot = last_plot(), width = 8, height = 6)
```
...
ggplot(df, aes(x = publish, fill = factor(low))) +
    ...
    labs(x = "Likilihood to Publish",
         y = "Density",
         fill = "Low") +
    theme_minimal()
# Save the plot
ggsave(filename = paste0(path_out, "publish-density.png"), plot = last_plot(), width = 8, height = 6)
```
...
# K-S Test 
ks <- ks.test(df$publish[df$low == 1], df2$publish[df$low == 0])
# Follows a beta distribution 
beta <- dbeta(df2$publish, 2, 5)
...
ggplot(df, aes(publish, fill = as.factor(low))) + 
    ...
    theme_minimal() 
# Save the plot
ggsave(filename = paste0(path_out, "publish-hist.png"), plot = last_plot(), width = 8, height = 6)
...
# Replot 
ggplot(df, aes(publish, fill = as.factor(low))) + 
    geom_histogram(alpha = 0.6, position = 'dodge', binwidth = 4, color = "white", linewidth = 0.2) # As presented
ggplot(df2, aes(publish, fill = as.factor(low))) + 
    geom_histogram(alpha = 0.6, position = 'dodge', binwidth = 4, color = "white", linewidth = 0.2) # With overlay
```
...
```{r}
# Change name to make stargazer work. 
df[, treat := low]
# OG
col1 <- plm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval, 
                data = df,
                index = c("id", "vignette"), 
                model = "within")
col1_se <- sqrt(diag(vcovHC(col1, type = "HC1", cluster = "group")))
...
# Vigenette 1
col2 <- lm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval + id, 
                data = df[df$vignette == 1,])
col2_se <- sqrt(diag(vcovHC(col2, type = "HC1", cluster = "group")))
    # TODO: Issue with adding clustering 
df_control <- subset(df, df$vignette == 1 & df$low == 0)
col2_mean <- round(mean(df_control$publish), 3) # Subset for control


# Vigenette 2
col3 <- lm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval + id, 
                data = df[df$vignette == 2,])
col3_se <- sqrt(diag(vcovHC(col3, type = "HC1", cluster = "group")))
    # TODO: Issue with adding clustering 
df_control <- subset(df, df$vignette == 2 & df$low == 0)
col3_mean <- round(mean(df_control$publish), 3) # Subset for control


# Vigenette 3
col4 <- lm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval + id, 
                data = df[df$vignette == 3,])
col4_se <- sqrt(diag(vcovHC(col4, type = "HC1", cluster = "group")))
    # TODO: Issue with adding clustering 
df_control <- subset(df, df$vignette == 3 & df$low == 0)
col4_mean <- round(mean(df_control$publish), 3) # Subset for control


# Vigenette 4
col5 <- lm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval + id, 
                data = df[df$vignette == 4,])
col5_se <- sqrt(diag(vcovHC(col5, type = "HC1", cluster = "group")))
    # TODO: Issue with adding clustering 
df_control <- subset(df, df$vignette == 4 & df$low == 0)
col5_mean <- round(mean(df_control$publish), 3) # Subset for control


# Vigenette 5
col6 <- lm(publish ~ treat + exlow + exhigh + field + phd + unilow + pval + id, 
                data = df[df$vignette == 5,])
col6_se <- sqrt(diag(vcovHC(col5, type = "HC1", cluster = "group")))
    # TODO: Issue with adding clustering 
df_control <- subset(df, df$vignette == 5 & df$low == 0)
col6_mean <- round(mean(df_control$publish), 3) # Subset for control


# Present 
stargazer(col1, col2, col3, col4, col5, col6,
        type = "text", 
        keep = c("treat"),
        covariate.labels = c("Null result treatment"),
        se = list(col1_se, col2_se, col3_se, col4_se, col5_se, col6_se), 
        keep.stat = c("n", "adj.rsq"),
        model.numbers = TRUE,
        digits = 3, 
        add.lines = list(c("Mean Dep. Var.", col1_mean, col2_mean, col3_mean, col4_mean, col5_mean, col6_mean),
                            c("Vigenettes", "All", "1st", "2nd", "3rd", "4th", "5th")),
        out = paste0(path_out, "table3_vigenettes.tex")) 
```
